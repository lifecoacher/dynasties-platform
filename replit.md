...
- **Active Financing Platform:** Overdue invoices with `financeEligible=true` can be financed via the "Get Paid Now" flow. Financing engine (`artifacts/api-server/src/services/financing-engine.ts`) computes terms: 2.5% provider rate + 0.5% platform spread = 3.0% customer rate. State machine: NONE→OFFERED→FUNDED→REPAID with validation. Three API endpoints: `offer-financing`, `accept-financing`, `mark-repaid`. UI shows animated "Get Paid Now" panel on overdue invoices with advance amount, fee breakdown, and accept CTA. Post-financing: FINANCED status badge, advance confirmation, and "Mark Repaid" button. Platform Revenue KPI card on overview dashboard tracks earnings from financing spreads.
