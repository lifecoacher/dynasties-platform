...
- **Stripe Billing & Subscription Management:** Full Stripe integration for SaaS subscription management. Products seeded via Stripe API (Starter $49/mo, Growth $149/mo, Scale $399/mo, Enterprise $999/mo + annual pricing). Webhook handler processes Stripe events via `stripe-replit-sync` (raw body before `express.json()`). Companies table extended with billing fields (stripeCustomerId, stripeSubscriptionId, billingStatus, planType, seatLimit, shipmentLimitMonthly, usage tracking). Stripe client: `artifacts/api-server/src/stripeClient.ts`. Service: `services/stripe-service.ts`. API: GET `/stripe/plans`, GET `/stripe/subscription`, POST `/stripe/checkout`, POST `/stripe/portal`, POST `/stripe/activate-demo`, GET `/stripe/publishable-key`. Plan enforcement middleware: `middlewares/billing-enforcement.ts` (bypassed in DEMO_MODE). Demo activation endpoint for instant plan switching without payment. UI: `/settings/billing` (SubscriptionBilling.tsx) — plan cards with monthly/yearly toggle, usage bars for seats/shipments with 80% warning, Stripe customer portal link, demo activation buttons. Products seed script: `scripts/seed-stripe-products.ts`. Plan limits: STARTER(3 seats/50 shipments), GROWTH(10/250), SCALE(25/1000), ENTERPRISE(unlimited).
