...
- **Active Financing Platform:** Any invoice with `financeEligible=true` and status OVERDUE/SENT/PARTIALLY_PAID (or financeStatus=OFFERED) can be financed via the "Get Paid Now" flow. Financing engine (`artifacts/api-server/src/services/financing-engine.ts`) computes terms: 2.5% provider rate + 0.5% platform spread = 3.0% customer rate. State machine: NONE→OFFERED→FUNDED→REPAID (APPROVED state removed for consistency). Three API endpoints: `offer-financing`, `accept-financing`, `mark-repaid`. UI shows animated "Get Paid Now" panel with advance amount, fee breakdown, and accept CTA. Post-financing: FINANCED status badge (green), advance confirmation, and "Mark Repaid" button. Accept-financing clears receivable (outstanding=0, collectionsStatus=CURRENT, settlementStatus=SETTLED). KPIs: Overdue (red, first position, emphasized), Disputed, Outstanding, Collected, Financed, Revenue Earned (renamed from "Platform Revenue"). Invoice list sorted by urgency (OVERDUE→DISPUTED→PARTIALLY_PAID→SENT→DRAFT), with "Days" column and relative date formatting. FINANCED badge styled green. Source column removed. Audit trail in chronological order. Pluralization fixes throughout.
