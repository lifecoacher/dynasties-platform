...
- **Routing & Pricing Agent:** `POST /shipments/:id/routing-pricing` generates optimal route options with cost estimates and risk-aware recommendations. Service at `services/routing-pricing/` with 4 modules: `routes.ts` (17 ports with lat/lng/region, transshipment hub mappings, haversine distance estimation, generates DIRECT + TRANSSHIPMENT + ALTERNATIVE routes), `pricing.ts` (weight/volume-based freight calculation, region multipliers, commodity surcharges, terminal handling, BAF, documentation fees, insurance — returns cost breakdown with confidence), `risk.ts` (evaluates compliance status, risk scores, doc validation gaps, jurisdiction risk, port congestion, dual-use HS codes, high-value cargo, transshipment handling, transit duration), `agent.ts` (AI reasoning with Anthropic Claude haiku for route ranking/explanation, deterministic fallback scoring). Output: recommended route index, route options with legs/cost/breakdown/advantages/disadvantages, risk factors with severity, recommendation summary, reasoning. Persisted to `routing_pricing_results` table with unique shipment constraint. `GET /shipments/:id/routing-pricing` returns latest result. UI: expandable route cards with cost breakdown, risk factors panel, analysis reasoning. All queries tenant-scoped. OPERATOR+ role. Audit event: ROUTING_PRICING_COMPLETED.
