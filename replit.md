...
- **Workflow Task & Case Management (Phase 4A):** Recommendation-to-action pipeline. DB schema: `workflow_tasks` table (9 task types: COMPLIANCE_CASE, PRICING_REVIEW, CARRIER_REVIEW, ROUTE_REVIEW, INSURANCE_REVIEW, DOCUMENT_CORRECTION_TASK, DISRUPTION_RESPONSE_TASK, RISK_MITIGATION_TASK, DELAY_RESPONSE_TASK; 5 statuses: OPEN, IN_PROGRESS, BLOCKED, COMPLETED, CANCELLED) and `task_events` audit trail table (9 event types: CREATED, ASSIGNED, STATUS_CHANGED, PRIORITY_CHANGED, NOTE_ADDED, DUE_DATE_CHANGED, COMPLETED, CANCELLED, REOPENED). API endpoints: `POST /api/recommendations/:id/create-task` (recommendation-to-task conversion with type mapping, priority inference, auto-due-date), `POST /api/tasks` (manual task creation), `GET /api/tasks` (work queue with filters: status, taskType, assignedTo, queue, overdue), `GET /api/tasks/summary` (aggregate stats by status/type/priority), `GET /api/tasks/:id` (task detail with audit trail), `PATCH /api/tasks/:id` (status transitions, assignment, priority changes, notes — all with audit events), `GET /api/shipments/:id/tasks` (shipment tasks). UI: Work Queue page (`/work-queue`) with summary cards, queue tabs (All/My/Overdue/Compliance/Pricing/Carrier-Route/Insurance/Documents/Disruption), status filters, task list with priority/status badges, task detail panel with action buttons (Start/Block/Unblock/Cancel/Complete), notes, and audit trail. "Create Task" button on decided recommendations in ShipmentDetail. Tests: `services/decision-engine/src/__tests__/phase4a.test.ts` (20 tests).
