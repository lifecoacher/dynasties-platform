...
function deriveVoice(alerts: any[], alertsSummary: any) {
  ...
  if (criticalAlerts > 0) return { text: `${criticalAlerts} critical risk${criticalAlerts > 1 ? "s" : ""} active`, color: "text-red-400" };
  if (alerts.length > 0) return { text: `${alerts.length} shipment${alerts.length > 1 ? "s" : ""} need attention`, color: "text-[#D4A24C]" };
  if (needsAttention > 0) return { text: `${needsAttention} item${needsAttention > 1 ? "s" : ""} to review`, color: "text-foreground/70" };
  return { text: "System operating normally", color: "text-primary/70" };
}
    ...
    <aside className="w-[280px] h-screen flex flex-col bg-sidebar border-l border-sidebar-border shrink-0 sticky top-0">
      <div className="px-5 py-4 border-b border-sidebar-border">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] font-semibold text-primary uppercase tracking-[0.15em]">Intelligence</span>
        </div>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="px-5 py-5">
                ...
                {alerts.map((s: any) => {
                  ...
                  return (
                    <Link key={s.id} href={`/shipments/${s.id}`}>
                      <div className="flex items-center gap-2.5 py-2 px-2.5 -mx-2.5 rounded-lg hover:bg-white/[0.03] transition-all cursor-pointer group active:scale-[0.98]">
          ...
          {summaries.length > 0 && (
            <>
              <p className="text-[11px] font-semibold text-muted-foreground uppercase tracking-[0.12em] mb-3">Recent Activity</p>
                  ...
                  >
                    <div className="pt-3 space-y-0.5">
                      {events.slice(0, 15).map((event: any) => (
                        <div
                          key={event.id}
                          className="flex items-start gap-2 py-1.5 text-[11px] opacity-60"
                        >
