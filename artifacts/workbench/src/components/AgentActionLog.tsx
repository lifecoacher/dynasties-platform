...
import { Bot, FileText, Shield, TrendingUp, User, Edit3, CheckCircle, XCircle } from "lucide-react";
...
export function AgentActionLog({ events }: { events?: Event[] }) {
  ...
  const getIcon = (type: string) => {
    ...
    if (type.includes('COMPLIANCE')) return <Shield className="w-4 h-4 text-warning" />;
    if (type.includes('RISK')) return <TrendingUp className="w-4 h-4 text-destructive" />;
        ...
        {events.map((event, index) => (
          ...
          >
            ...
            <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded-xl border border-border/50 bg-secondary/20 shadow-sm transition-colors hover:border-primary/30">
              ...
              <div className="text-xs text-muted-foreground/80 mt-2 font-mono bg-background/50 p-2 rounded">
                {event.serviceId || event.userId || 'System'}
              </div>
            </div>
          </motion.div>
        ))}
