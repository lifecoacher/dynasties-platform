...
const EXCEPTION_TO_TASK: Record<string, { taskType: string; priority: string }> = {
  CUSTOMS_HOLD: { taskType: "COMPLIANCE_CASE", priority: "CRITICAL" },
  RELEASE_BLOCKED: { taskType: "COMPLIANCE_CASE", priority: "CRITICAL" },
  OSD_DAMAGE_SHORTAGE: { taskType: "RISK_MITIGATION_TASK", priority: "CRITICAL" },
  DELAYED_SHIPMENT: { taskType: "DELAY_RESPONSE_TASK", priority: "HIGH" },
  MISSED_PICKUP: { taskType: "DISRUPTION_RESPONSE_TASK", priority: "HIGH" },
  HIGH_RISK_REVIEW: { taskType: "COMPLIANCE_CASE", priority: "HIGH" },
  MAJOR_INVOICE_VARIANCE: { taskType: "PRICING_REVIEW", priority: "HIGH" },
  MISSING_DOCUMENTS: { taskType: "DOCUMENT_CORRECTION_TASK", priority: "MEDIUM" },
  DOCUMENT_BLOCKED: { taskType: "DOCUMENT_CORRECTION_TASK", priority: "HIGH" },
  UNMATCHED_CARRIER_INVOICE: { taskType: "PRICING_REVIEW", priority: "MEDIUM" },
  DELIVERY_EXCEPTION: { taskType: "DISRUPTION_RESPONSE_TASK", priority: "HIGH" },
  ...
  for (const exc of activeExceptions) {
    const existing = await db
      ...
      .where(
        and(
          eq(workflowTasksTable.companyId, companyId),
          sql`${workflowTasksTable.metadata}->>'exceptionId' = ${exc.id}`,
          inArray(workflowTasksTable.status, ["OPEN", "IN_PROGRESS", "BLOCKED"]),
        ),
      )
      .limit(1);
    ...
    const mapping = EXCEPTION_TO_TASK[exc.exceptionType] || { taskType: "RISK_MITIGATION_TASK", priority: "MEDIUM" };
    const taskId = generateId("tsk");
    const eventId = generateId("tev");

    const dueHours = mapping.priority === "CRITICAL" ? 4 : mapping.priority === "HIGH" ? 24 : 72;
    ...
    await db.transaction(async (tx) => {
      await tx.insert(workflowTasksTable).values({
        ...
        title: exc.title,
        description: exc.description || `Auto-generated from ${exc.exceptionType} exception`,
        status: "OPEN",
        priority: mapping.priority as any,
        createdBy: userId,
        dueAt: new Date(Date.now() + dueHours * 60 * 60 * 1000),
        executionNotes: exc.recommendedAction,
        metadata: { exceptionId: exc.id, exceptionType: exc.exceptionType, autoGenerated: true },
      });
      ...
      await tx.insert(taskEventsTable).values({
        ...
        actorId: userId,
        afterValue: "OPEN",
        notes: `Auto-generated from ${exc.severity} ${exc.exceptionType} exception: ${exc.title}`,
      });
    });
