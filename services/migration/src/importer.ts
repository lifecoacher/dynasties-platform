...
export async function importData(
  ...
  await db.transaction(async (tx) => {
    ...
    if (existingAccount) {
      ...
      await tx.insert(billingAccountsTable).values({
        id: billingAccountId,
        companyId,
    ...
    for (const entity of customers) {
      try {
        ...
        if (billingAccountId) {
          ...
          await tx.insert(customerBillingProfilesTable).values({
            id: profileId,
            companyId,
            billingAccountId,
            entityId: id,
  ...
  for (const shipmentId of createdShipmentIds) {
    try {
      await createShipmentCreatedEvent(companyId, shipmentId);
    } catch (evtErr: any) {
      console.warn(`[importer] SHIPMENT_CREATED event failed for ${shipmentId}: ${evtErr.message}`);
    }
  }
